...
Iso2709.format = function(record) {
  ...
  chunks.push(new Buffer.from(RT));
  directory += FT;
  ...
  leader = intpadded(length,5) + leader.substr(5,7) + intpadded(offset,5) +
       leader.substr(17);
  chunks[0] = new Buffer.from(leader);
  chunks[1] = new Buffer.from(directory);
