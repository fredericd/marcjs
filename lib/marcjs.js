...
var Iso2709Reader = function(stream) {
  ...
  this._emitDataEvent = function(record) {
    self.count++;
    self.emit('data', record);
  };

  this._processData = function(doneCallback) {
    if (self.ended || self.data.length === 0) {
      if (typeof doneCallback !== 'undefined') {
        doneCallback();
      }
    } else {
      // Skip over any initial undefined or empty chunks in self.data
      let firstChunk = 0;
      while ((typeof self.data[firstChunk] === 'undefined' || self.data[firstChunk].length === 0)
             && firstChunk < self.data.length) {
        firstChunk++;
      }
      // Find in which chunk the current record ends (lastChunk), and where in the last chunk it ends (endOfRecordInLastChunk)
      let lastChunk = firstChunk;
      let endOfRecordInLastChunk = -1;
      if (lastChunk < self.data.length) {
        while (endOfRecordInLastChunk === -1 && lastChunk < self.data.length) {
          endOfRecordInLastChunk = self.data[lastChunk].indexOf(0x1D);
          if (endOfRecordInLastChunk === -1) {
            lastChunk++;
          }
        }
      }
      if (endOfRecordInLastChunk === -1) {
        ...
        if (firstChunk === lastChunk && endOfRecordInLastChunk === self.data[firstChunk].length - 1) {
          self._emitDataEvent(self.parse(self.data[firstChunk]));
          self.data = self.data.slice(firstChunk + 1);
        } else {
          // Concatenate any whole chunks and the part of the last chunk that are part of this record
          let thisRecord = Buffer.concat([
                                           ...self.data.slice(firstChunk, lastChunk),
                                           self.data[lastChunk].slice(0, endOfRecordInLastChunk + 1),
                                         ]);
          // Parse the current record
          self._emitDataEvent(self.parse(thisRecord));
          // Set self.data to be the remaining part of the last chunk and the remaining whole chunks
          self.data = [self.data[lastChunk].slice(endOfRecordInLastChunk + 1), ...self.data.slice(lastChunk + 1)];
        }
        if (self.ended || self.data.length === 0) {
          if (typeof doneCallback !== 'undefined') {
            doneCallback();
          ...
          // In the next tick, process more of the remaining data
          setImmediate(self._processData, doneCallback);
        }
      }
    }
  };
