...
class Iso2709 extends Duplex {
  ...
  static parse(data, encoding) {
    const record = new Record();
    record.leader = data.toString(encoding, 0, 24);
    const directoryLen = parseInt(data.toString(encoding, 12, 17), 10) - 25;
    const numberOfTag = directoryLen / 12;
    record.fields = [];
    for (let i = 0; i < numberOfTag; i += 1) {
      const off = 24 + i * 12;
      const tag = data.toString(encoding, off, off + 3);
      const len = parseInt(data.toString(encoding, off + 3, off + 7), 10) - 1;
      const pos = parseInt(data.toString(encoding, off + 7, off + 12), 10) + 25 + directoryLen;
      let value = data.toString(encoding, pos, pos + len);
      const parts = [tag];
